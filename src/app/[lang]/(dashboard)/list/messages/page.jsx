...
export default function Messenger() {
  ...
  useEffect(() => {
    if (!pusherRef.current && user?.id) {
      ...
      const presenceChannel = pusherRef.current.subscribe('presence-users');
      
      presenceChannel.bind('pusher:subscription_succeeded', (members) => {
        members.each((member) => setOnlineUsers(prev => new Set(prev).add(member.id)));
        console.log("Subskrypcja udana, użytkownicy online:", members.count);
      });

      presenceChannel.bind('pusher:member_added', (member) => {
        setOnlineUsers(prev => new Set(prev).add(member.id));
        console.log("Użytkownik dołączył:", member.id);
      });
      ...
      presenceChannel.bind('pusher:member_removed', (member) => {
        ...
        console.log("Użytkownik opuścił:", member.id);
      });
      ...
      const privateChannel = pusherRef.current.subscribe(`private-user-${user.id}`);
