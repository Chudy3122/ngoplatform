...
const EnergyPriceChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || t.energyChart.error);
        }
    ...
    fetchData();
    const interval = setInterval(fetchData, 300000); // Odświeżanie co 5 minut
    return () => clearInterval(interval);
  }, [retryCount, t]);

  if (isLoading) {
    return (
      <div className="bg-white p-4 rounded-lg h-96 flex items-center justify-center">
  ...
  if (error) {
    return (
      <div className="bg-white p-4 rounded-lg h-96 flex flex-col items-center justify-center gap-4">
